function [selectedFeatures] = JMI4(data,labels, topK, arities)
% Summary
%    JMI-4 algorithm for feature selection
% Inputs
...
for index_feature_ns = 1:length(not_selected_features)
    ...
    % score_per_feature(not_selected_features(index_feature_ns)) = score_per_feature(not_selected_features(index_feature_ns))+cmi_Ind_JS(data(:,not_selected_features(index_feature_ns)), labels,data(:, selectedFeatures(count-1)), arities(not_selected_features(index_feature_ns)),arities(end),arities(selectedFeatures(count-1)));
...
score_per_feature = zeros(1,numFeatures);
score_per_feature(selectedFeatures(1:3)) = NaN;
count = 4;
while count<=topK
    ...
    for index_feature_ns = 1:length(not_selected_features)
        ...
        for index_feature_s1 = 1:(length(selectedFeatures)-2)
            ...
            for index_feature_s2 = (index_feature_s1+1):(length(selectedFeatures)-1)
                ...
                score_per_feature(not_selected_features(index_feature_ns)) = score_per_feature(not_selected_features(index_feature_ns))+mi_Ind_JS([data(:,not_selected_features(index_feature_ns)),data(:, selectedFeatures(count-1)),data(:, selectedFeatures(index_feature_s1)),data(:, selectedFeatures(index_feature_s2))], labels,arities([not_selected_features(index_feature_ns) selectedFeatures(count-1) selectedFeatures(index_feature_s1) selectedFeatures(index_feature_s2)]),arities(end));
                % score_per_feature(not_selected_features(index_feature_ns)) = score_per_feature(not_selected_features(index_feature_ns))+cmi_Ind_JS([data(:,not_selected_features(index_feature_ns))], labels,[data(:, selectedFeatures(count-1)) data(:, selectedFeatures(index_feature_s))], arities([not_selected_features(index_feature_ns)]),arities(end),arities([selectedFeatures(count-1) selectedFeatures(index_feature_s)]));
            end
        end
